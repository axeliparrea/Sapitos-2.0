...
const PedirProducto = () => {
  ...
  const handleCrearOrden = () => {
    alert("Aquí se mostraría un formulario para crear orden.");
  };

  if (loading) return <div className="text-center p-4">Cargando órdenes...</div>;
  ...
  return (
    <div className='card h-100 p-0 radius-12'>
      <div className='card-header d-flex justify-content-between align-items-center py-16 px-24'>
        ...
        <Button variant="success" size="sm" onClick={handleCrearOrden}>
      ...
      <div className='card-body p-24'>
                ...
                <React.Fragment key={p.ORDEN_ID}>
                  ...
                  {detalles[p.ORDEN_ID]?.map((item, i) => (
                    <tr key={`detalle-${p.ORDEN_ID}-${i}`} className="bg-light">
                      ...
                      <td>Precio: ${item.PrecioUnitario.toFixed(2)}</td>
                      <td colSpan="2">Total: ${(item.Cantidad * item.PrecioUnitario).toFixed(2)}</td>
                    </tr>
                  ))}
                </React.Fragment>
              )) : (
        ...
        <div className="d-flex justify-content-between align-items-center mt-3">
          <small className="text-muted">
            Mostrando {idxFirst + 1} a {Math.min(idxLast, filtered.length)} de {filtered.length} órdenes
          </small>
          <div>
