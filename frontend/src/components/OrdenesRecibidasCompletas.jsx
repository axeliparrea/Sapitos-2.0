  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filtrar solo las órdenes con estado "Completado"
const ordenesCompletadas = res.data.filter(o =>
  o.ESTADO?.toLowerCase() === 'completado'
);

setPedidos(ordenesCompletadas);
setFiltered(ordenesCompletadas);
          ...
          <Table bordered hover size="sm">
            ...
            <tbody>
              {currentItems.length > 0 ? currentItems.map(p => (
                ...
                <tr><td colSpan="7" className="text-center">No se encontraron pedidos</td></tr>
              )}
            </tbody>
          </Table>
        </div>
